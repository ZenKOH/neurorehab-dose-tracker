...
function report() {
  ...
  $("reportPreview").innerHTML = ps.length
    ? `<div class="report-header"><div><h3>Neurorehabilitation Dose Review</h3><p>Generated ${fmt(today())} · ${ps.length} programme(s) in view</p></div><span class="badge info">Prototype report</span></div><div class="report-columns">${ps
        .map((c) => {
          const s = stats(c);
          const equipment = [
            ...new Map(
...
function csv() {
  ...
  dl(
    `neurorehab-dose-tracker-${today()}.csv`,
    [h.join(","), ...rows].join("\n"),
...
function fhir() {
  ...
  state.cases.forEach((c) => {
    entry.push({
      resource: {
        ...
        identifier: [
          {
            system: "https://zenkoh.github.io/neurorehab-dose-tracker/case",
            value: c.label,
          },
        ],
  ...
  state.equipment.forEach((item) => {
    ...
    if (item.identifier)
      resource.identifier = [
        {
          system: "https://zenkoh.github.io/neurorehab-dose-tracker/equipment",
  ...
  dl(
    `neurorehab-fhir-shaped-export-${today()}.json`,
    JSON.stringify(
      {
        resourceType: "Bundle",
        language,
...
function bind() {
  ...
  $("exportNoteBtn")?.addEventListener("click", () =>
    dl(`neurorehab-progress-note-${today()}.txt`, note()),
  );
  $("exportFhirBtn")?.addEventListener("click", fhir);
  $("backupBtn")?.addEventListener("click", () =>
    dl(
      `neurorehab-dose-tracker-backup-${today()}.json`,
