...
test("exposes all six supported languages", () => {
  assert.deepEqual(Object.keys(global.i18n.languages), ["en", "zh-Hans", "es", "fr", "de", "ms"]);
});
...
test("localises dynamic dose and clinical-review messages", () => {
  ...
  assert.equal(global.i18n.translateText("Target: 180 min · 75%"), "Objetivo: 180 min · 75%");
  assert.match(global.i18n.translateText("45 active minutes against a 180 minute target."), /45 minutos activos/);

  global.i18n.setLanguage("zh-Hans");
  assert.equal(global.i18n.translateText("3 session(s) show pain >=7/10."), "3 次治疗显示疼痛 ≥7/10。");

  global.i18n.setLanguage("ms");
  assert.equal(global.i18n.translateText("Programme saved."), "Program disimpan.");
});
...
test("falls back safely for unsupported languages and user text", () => {
  ...
  assert.equal(global.i18n.translateText("Patient-authored goal text"), "Patient-authored goal text");
});
